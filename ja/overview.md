...
ロードバランサー(DSR)の VIP(Virtual IP) は、ロードバランサーが属するサブネットから割り当てられる**プライベート IP** であり、ロードバランサー(DSR)の作成時に **[VIP(Virtual IP)]** 項目で指定するアドレスで、一覧の **[IP アドレス]** 列に表示されます。インターネットからアクセスする際に使用する Floating IP とは別のアドレスです。

| 区分 | アドレス例 | 役割 | メンバーサーバーの lo および追加許可アドレスの設定 |
|------|----------|------|------|
...
2. リクエストの分散: ロードバランサーが適切なメンバーインスタンスを選択し、宛先IPをVIPのまま維持した状態でリクエストを転送します。
3. レスポンスの直接送信: メンバーインスタンスがVIPを送信元としてロードバランサーを経由せずにクライアントに直接レスポンスを送信します。Floating IPで受信したリクエストへのレスポンスは、送信元が再びFloating IPに変換されてクライアントに届きます。

!!! danger "注意"
    メンバーサーバーのloインターフェースおよびネットワークインターフェースの追加許可アドレスに登録する必要があるアドレスはVIP（プライベートIP）です。Floating IPを登録すると、トラフィックが正常に処理されません。
...
- TCP: 指定されたポートへの TCP 接続を試み、接続可能かどうかを確認します。特定のサービスポートが正常に動作しているかどうかを確認できます。リクエストはロードバランサー(DSR)の VIP(プライベート IP)を宛先として送信されます。

- HTTP: 指定されたパスにHTTPリクエストを送信し、レスポンスコードを確認します。Webアプリケーションの実際のサービス状態をより正確に確認できます。リクエストはロードバランサー(DSR)のVIP(プライベートIP)を宛先として送信されます。

!!! tip "ポイント"
    TCP/HTTP ヘルスチェックパケットの宛先は Floating IP ではなく VIP であり、これは正常な動作です。TCP/HTTP ヘルスチェックは DSR VIP を宛先としてリクエストを送信するため、メンバーサーバーのloインターフェースに VIP が設定されていない場合、該当パケットを受信・処理できずヘルスチェックが失敗し、メンバーが `INACTIVE` と判定されます。これは、サーバー側の VIP 設定の欠落を早期に検出するための動作です。ICMP ヘルスチェックはメンバーの実際の IP アドレスにリクエストを送信するため、VIP の設定に関係なく接続性のみを確認します。
...
ロードバランサー(DSR)は、[VPC](/Network/VPC/ja/overview{% if "gov" in build_flags %}-gov{% endif %}/#glossary)の[サブネット](/Network/VPC/ja/overview{% if "gov" in build_flags %}-gov{% endif %}/#glossary)内に作成されます。

<a id="assign-vip-address"></a>
### VIPアドレスの割り当て { #assign-vip-address }

VIP(Virtual IP)は、ロードバランサー(DSR)が属するサブネットから割り当てられるプライベートIPです。ロードバランサー(DSR)を作成する際、次の2つの方式でVIPを割り当てることができます。

- 自動割り当て: サブネットの利用可能なIPのいずれかを自動で割り当ててVIPとして使用します。
- 直接指定: サブネットのCIDR範囲内で任意のIPを指定してVIPとして使用します。

インターネットからアクセスする必要がある場合は、このVIPにFloating IPを関連付けます。詳細については、[Floating IPの関連付け](#floating-ip-association)を参照してください。
...
ロードバランサー(DSR)は、クライアントのリクエストをVIP(Virtual IP)を宛先としてメンバーサーバーに転送します。メンバーサーバーがこのパケットを正常に受信して応答するには、サーバー側で以下の設定が必要です。以下の設定で使用する `<VIP>` はすべてこのプライベートIPです。
...
メンバーが `INACTIVE` 状態であるか、トラフィックを受信できない場合、メンバーインスタンスごとに以下の項目を順番に確認します。設定の未適用項目はメンバーによって異なる場合があります。以下の順序は設定の順序ではなく、診断の順序です。新規に設定する場合は、カーネルパラメータを先に適用する必要があります。

| 順序 | 確認項目 | 確認方法 | 正常状態 |
|------|----------|----------|----------|
| 1 | loインターフェースのVIP | `ip addr show lo` | VIP（プライベートIP）が`/32`で登録されています |
| 2 | カーネルパラメータ | `sysctl net.ipv4.conf.{all,lo}.arp_ignore`, `sysctl net.ipv4.conf.{all,lo}.arp_announce` | `arp_ignore=1`, `arp_announce=2` |
...
!!! tip "ポイント"
    ロードバランサー(DSR)のVIPとヘルスチェック専用IPに該当するポートには、default Security Groupが関連付けられています。ただし、DSRポート自体にはSecurity Groupsのフィルタリング(flow)は適用されません。

    メンバーインスタンスのポートには、Security Groupsのフィルタリングが正常に適用されます。この場合、DSRはパケットの送信元IPを変換しないため(No SNAT)、サービストラフィックの送信元IPはクライアントの元のIPとなります。そのため、サービストラフィックにはクライアントのIPアドレス範囲またはANY(`0.0.0.0/0`)をリモートとして指定する必要があります。

    ヘルスチェックトラフィックは、DSRと同じサブネットに割り当てられたヘルスチェック専用IPから送信されるため、該当サブネットのCIDRをリモートとして指定すると許可されます。ヘルスチェック専用IPのポートはdefault Security Groupにも属しているため、`default` SGをリモートとして指定する方法でも許可できます。
...
| 受信 | 任意 | - | DSR サブネット CIDR（例: `192.168.1.0/24`） | ヘルスチェックトラフィックを許可（ヘルスチェック専用IPがDSRと同じサブネットに割り当てられる） |
...
!!! danger "注意"
    ヘルスチェックトラフィックのリモートに `default` Security Group を指定することもできますが、この場合、default SG に属するすべてのリソースが該当ポートにアクセスできます。インスタンス作成時に Security Groups を指定しない場合、default SG が適用されるため、サブネットや用途 (web、app、db など) で区分したネットワーク境界が Security Group レベルで緩くなります。`default` SG の指定は初期テスト・検証目的のみで使用し、本番環境では DSR サブネット CIDR を指定する方式をお勧めします。
...
| TCP ヘルスチェック | TCP | `health_check_port` | DSR サブネット CIDR | ヘルスチェック専用IPがDSRと同じサブネットに割り当てられます。 |
| ICMPヘルスチェック | ICMP | - | DSRサブネットCIDR | ICMPタイプ使用時 |
| HTTPヘルスチェック | TCP | `health_check_port` | DSRサブネットCIDR | HTTPタイプ使用時 |

##### コンソールでのルール追加例

サービスポートとTCPヘルスチェックポートがどちらも80で、DSRサブネットが`192.168.1.0/24`の場合:

| 方向 | IPプロトコル | ポート範囲 | リモート | 説明 |
|------|-----------|----------|------|------|
| 受信 | TCP | 80 | 0.0.0.0/0 | クライアントからのサービストラフィック |
| 受信 | TCP | 80 | 192.168.1.0/24 | ヘルスチェックトラフィック |

ICMPヘルスチェックを使用する場合の追加:

| 方向 | IPプロトコル | ポート範囲 | リモート | 説明 |
|------|-----------|----------|------|------|
| 受信 | ICMP | - | 192.168.1.0/24 | ICMP ヘルスチェック |

!!! tip "ポイント"
    * ヘルスチェックポート（`health_check_port`）をサービスポートと異なる設定にした場合、Security Groups で両方のポートを許可する必要があります。
    * クライアント IP アドレス範囲が特定の CIDR（例: `10.0.0.0/8`）に限定されている場合、`0.0.0.0/0` の代わりに該当 CIDR を指定することで、最小権限の原則を適用できます。
    * 運用環境では、default の Security Group をそのまま使用するよりも、用途（web、app、db など）ごとに専用の Security Group を作成し、必要なルールのみを適用することをお勧めします。ヘルスチェックルールのリモートも `default` SG の代わりに DSR サブネット CIDR を指定することで、許可範囲を該当サブネットに絞り込むことができます。
...
!!! tip "ポイント"
    追加許可アドレスの設定手順については、[コンソール使用ガイド](/Network/Network%20Interface/ja/console-guide$[ ni_suffix ]$/)を参照してください。
...
Floating IPへの受信リクエストは、宛先がVIPに変換された後、ロードバランサーに転送されます。レスポンスは、送信元が再びFloating IPに変換されます。そのため、メンバーサーバーのloインターフェースおよびネットワークインターフェースの追加許可アドレスには、Floating IPではなくVIPを登録する必要があります。
