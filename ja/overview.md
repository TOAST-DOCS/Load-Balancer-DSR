...
ロードバランサー(DSR)の VIP(Virtual IP) は、ロードバランサーが属するサブネットから割り当てられる**プライベートIP** であり、ロードバランサー(DSR)の作成時に **[VIP(Virtual IP)]** 項目で指定するアドレスで、一覧の **[IP アドレス]** 列に表示されます。インターネットからアクセスする際に使用する Floating IP とは別のアドレスです。
...
- TCP: 指定されたポートへの TCP 接続を試み、接続可能かどうかを確認します。特定のサービスポートが正常に動作しているかどうかを確認できます。リクエストはロードバランサー(DSR)の VIP(プライベートIP)を宛先として送信されます。
