...
멤버 선택은 별도의 sticky table을 두지 않고 해시 기반의 일관된 매핑(Consistent Hashing)으로 이뤄집니다. 세션 지속성 비활성화 시에는 5-tuple, 활성화 시에는 소스 IP를 해시 입력으로 사용하며, 멤버 구성이 동일하다면 동일한 입력 키는 항상 동일한 멤버로 매핑됩니다. 또한 한 번 수립된 세션이 유효한 동안에는 해당 세션의 모든 패킷이 동일한 멤버로 전달되어, 세션 단위의 멤버 고정이 보장됩니다.
...
- IP 단편화 패킷 처리: IP 단편화(fragmentation)된 패킷은 L4 헤더(포트/플래그)를 확인할 수 없어 일관된 멤버 매핑이 불가능하므로 DROP됩니다. 클라이언트와 멤버 인스턴스의 MTU를 적절히 설정하거나 Path MTU Discovery가 정상 동작하도록 구성하여 단편화가 발생하지 않도록 하세요.
