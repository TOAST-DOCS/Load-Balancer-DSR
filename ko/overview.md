...
> [참고] 상태 확인 목적지와 VIP 설정<br>
> TCP/HTTP 상태 확인는 DSR VIP를 목적지로 요청하므로, 멤버 서버의 lo 인터페이스에 VIP가 설정되어 있지 않으면 해당 패킷이 수신·처리되지 못해 상태 확인가 실패하고 멤버가 `INACTIVE`로 판정됩니다. 이는 서버 측 VIP 설정 누락을 조기에 탐지하기 위한 동작입니다. ICMP 상태 확인는 멤버의 실제 IP로 요청하므로 VIP 설정과 무관하게 연결성만 확인합니다.
...
> [참고] 멤버 초기 상태<br>
> 새로 등록된 멤버의 초기 상태는 `INACTIVE`입니다. 헬스 모니터가 설정된 경우, 상태 확인를 통과하면 자동으로 `ACTIVE` 상태로 전환되어 트래픽을 수신하기 시작합니다.

> [참고] 동일 포트 중복 등록 불가<br>
...
> [참고] 상태 확인 요청의 출발지<br>
> 상태 확인 요청은 로드 밸런서의 VIP가 아닌 **헬스 모니터 전용 IP**에서 발송됩니다. Security Group 및 서버 내부 방화벽에서 해당 IP의 트래픽을 허용해야 합니다. 헬스 모니터 IP는 DSR과 동일한 서브넷에 자동으로 할당됩니다.

> [참고] 서버 내부 방화벽 확인<br>
> 서버에 iptables, firewalld 등 내부 방화벽이 설정되어 있는 경우, 서비스 포트와 상태 확인 포트(ICMP 포함)가 차단되지 않는지 확인하세요. 예를 들어 ICMP 상태 확인 사용 시 `iptables -A INPUT -p icmp --icmp-type echo-request -j ACCEPT`로 허용해야 합니다.

### 5. Security Group 설정

멤버 인스턴스의 Security Group에서 DSR로부터의 서비스 트래픽과 상태 확인 트래픽을 허용해야 합니다.

> [참고] DSR 포트와 Security Group의 관계<br>
...
>
> 반면, **상태 확인 트래픽**은 DSR과 동일한 서브넷에 할당된 헬스 모니터 전용 IP에서 발송되며, 해당 포트가 default SG에 속해 있으므로 `default` SG를 원격으로 지정하면 허용됩니다.
...
| 수신 | 임의 | - | default | 상태 확인 트래픽 허용 (헬스 모니터 포트가 default SG에 속함) |

> [참고] 서비스 트래픽에 ANY(0.0.0.0/0)를 사용하는 이유<br>
...
| TCP 상태 확인 | TCP | `health_check_port` | DSR 서브넷 CIDR 또는 default SG | 헬스 모니터 전용 IP에서 발송 |
...
ICMP 상태 확인를 사용하는 경우 추가:

| 방향 | IP 프로토콜 | 포트 범위 | 원격 | 설명 |
|------|-----------|----------|------|------|
| 수신 | ICMP | - | default | ICMP 상태 확인 |

> [참고] 서비스 포트와 상태 확인 포트가 다른 경우<br>
> 상태 확인 포트(`health_check_port`)를 서비스 포트와 다르게 설정한 경우, Security Group에서 두 포트를 모두 허용해야 합니다.

> [참고] 서비스 트래픽의 원격을 제한하는 경우<br>
> 클라이언트 IP 대역이 특정 CIDR(예: `10.0.0.0/8`)로 한정되어 있다면, `0.0.0.0/0` 대신 해당 CIDR을 지정하여 최소 권한 원칙을 적용할 수 있습니다.

> [참고] 상태 확인 원격을 CIDR로 지정하는 경우<br>
> 상태 확인 규칙에서 default Security Group 대신 서브넷 CIDR(예: `192.168.1.0/24`)로 지정할 수도 있습니다. 상태 확인 요청은 DSR과 동일한 서브넷에 자동 할당된 헬스 모니터 전용 IP에서 발송되므로, 서브넷 CIDR 단위로 허용하면 됩니다.
...
- [ ] Security Group에서 서비스 포트 및 상태 확인 트래픽 허용 확인
...
> [참고] 멤버 상태 자동 관리<br>
> 헬스 모니터가 설정된 경우, 멤버의 상태는 상태 확인 결과에 따라 자동으로 `ACTIVE` 또는 `INACTIVE`로 변경됩니다. 상태 확인 실패로 `INACTIVE` 상태가 된 멤버는 자동으로 트래픽 분산 대상에서 제외되며, 이후 상태 확인에 성공하면 다시 `ACTIVE` 상태로 전환되어 트래픽을 수신합니다. 멤버를 수동으로 비활성화하면 `ONLINE` 상태로 표시되며 트래픽 분산 대상에서 제외됩니다.

> [참고] 멤버 초기 상태<br>
> 새로 등록된 멤버는 `INACTIVE` 상태로 시작합니다. 헬스 모니터가 설정되어 있으면 상태 확인 통과 후 자동으로 `ACTIVE`로 전환됩니다.
