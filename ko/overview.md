...
Floating IP로 유입된 요청은 목적지가 VIP(사설 IP)로 변환되어 로드 밸런서로 전달되고, 응답은 출발지가 다시 Floating IP로 변환됩니다. 따라서 멤버 서버의 lo 인터페이스와 네트워크 인터페이스의 추가 허용 주소에는 Floating IP가 아닌 VIP를 등록해야 합니다.
