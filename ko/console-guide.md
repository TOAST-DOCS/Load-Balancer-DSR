...
* 서브넷: 로드 밸런서(DSR)가 속할 서브넷을 선택합니다. 로드 밸런서(DSR)와 멤버 인스턴스는 **동일한 서브넷에 위치**해야 합니다.
...
> [참고] 로드 밸런서(DSR)와 일반 로드 밸런서의 차이점<br>
...
로드 밸런서(DSR)에 등록된 멤버 인스턴스의 정상 동작 여부를 주기적으로 확인하기 위한 헬스 모니터를 설정합니다. 하나의 로드 밸런서(DSR)에는 헬스 모니터를 1개만 설정할 수 있습니다.
...
> [참고] 헬스 체크 목적지와 VIP 설정<br>
> TCP/HTTP 헬스 체크는 DSR VIP를 목적지로 요청하므로, 멤버 서버의 lo 인터페이스에 VIP가 설정되어 있지 않으면 해당 패킷이 수신·처리되지 못해 헬스 체크가 실패하고 멤버가 `INACTIVE`로 판정됩니다. ICMP 헬스 체크는 멤버의 실제 IP로 요청하므로 VIP 설정과 무관하게 연결성만 확인합니다.

#### 3. 멤버 설정

로드 밸런서(DSR) 생성 시 등록할 멤버 인스턴스를 지정합니다. 멤버 등록은 로드 밸런서(DSR) 생성 후에도 할 수 있습니다.

* 인스턴스 선택: 로드 밸런서(DSR)와 **동일한 서브넷**에 속한 인스턴스(네트워크 인터페이스)를 선택합니다. 하나 이상의 인스턴스를 동시에 선택하여 멤버로 등록할 수 있습니다.

> [참고] DSR 멤버는 L4 포트를 지정하지 않습니다<br>
> 로드 밸런서(DSR)는 클라이언트 요청의 **목적지 포트(VIP 포트)를 그대로 유지**한 채 멤버 인스턴스로 전달합니다. 따라서 일반 로드 밸런서와 달리 멤버 등록 시 멤버별 서비스 포트를 지정하지 않으며, 멤버 인스턴스의 네트워크 인터페이스만 선택합니다. 멤버 서버의 애플리케이션은 `0.0.0.0` 또는 VIP에 바인딩되어 클라이언트가 보낸 동일한 포트를 리슨해야 합니다.

> [주의] 멤버 서버 측 설정 필요<br>
> 멤버 인스턴스가 VIP로 오는 트래픽을 정상적으로 수신·응답하려면 서버 내부에서 다음 설정이 필요합니다.
> - 네트워크 인터페이스에 **VIP를 허용된 주소 쌍(Allowed Address Pair)으로 추가** (콘솔 Network Interface 메뉴)
> - 커널 파라미터 설정 (`arp_ignore=1`, `arp_announce=2`)
> - lo 인터페이스에 VIP를 `/32` 서브넷으로 추가
> - Security Group에서 서비스 포트 및 헬스 체크 트래픽 허용
>
> 자세한 절차는 **[로드 밸런서(DSR) 개요](/Network/Load%20Balancer%20(DSR)/ko/overview/)**의 멤버 서버 설정 가이드를 참고하시기 바랍니다.

> [참고] 멤버 초기 상태<br>
> 새로 등록된 멤버의 초기 상태는 `INACTIVE`입니다. 헬스 모니터가 설정된 경우, 헬스 체크를 통과하면 자동으로 `ACTIVE` 상태로 전환되어 트래픽을 수신합니다.
...
> [참고] 로드 밸런서(DSR)의 상태는 다음 중 하나로 결정됩니다.
>
> | 상태 | 의미 |
> |--|--|
> | `ACTIVE` | 정상 동작 중 |
> | `BUILD` | 로드 밸런서(DSR) 생성 중 |
> | `ERROR` | 오류 발생. 관리자에게 문의하시기 바랍니다. |
...
> [주의] 로드 밸런서(DSR) 삭제 시 리소스 정리<br>
> 로드 밸런서(DSR)를 삭제하면 해당 DSR에 등록된 **모든 멤버와 헬스 모니터가 함께 삭제**됩니다. 플로팅 IP가 연결된 경우 자동으로 해제됩니다.
...
* 포트 ID: 멤버 인스턴스의 네트워크 인터페이스 식별자입니다.
* 접속 IP: 멤버 인스턴스의 IP 주소입니다.
* 포트 이름: 멤버로 등록된 네트워크 포트의 이름입니다.
* 인스턴스 이름: 멤버로 등록된 인스턴스의 이름입니다.
* 상태: 멤버의 현재 상태입니다.

> [참고] DSR은 멤버별 서비스 포트를 가지지 않습니다<br>
> 로드 밸런서(DSR)는 클라이언트 요청의 목적지 포트를 그대로 유지한 채 멤버로 전달하므로, 멤버 항목에는 L4 서비스 포트가 따로 표시되지 않습니다. 실제 서비스 포트는 클라이언트가 VIP로 요청한 포트이며, 멤버 서버의 애플리케이션이 해당 포트를 리슨(Listen)해야 합니다.
...
> [참고] DSR 멤버 추가 시 L4 포트 입력이 없습니다<br>
...
> [주의] 멤버 등록 제약 사항
> * 멤버 인스턴스는 로드 밸런서(DSR)와 **동일한 서브넷**에 속해야 합니다.
> * 컴퓨트 인스턴스만 멤버로 등록할 수 있습니다.
> * 동일한 인스턴스 포트를 같은 로드 밸런서(DSR)에 중복으로 등록할 수 없습니다.
> * 하나의 로드 밸런서(DSR)에는 기본적으로 최대 30개의 멤버를 등록할 수 있습니다.

> [참고] 멤버 서버 측 설정<br>
> 멤버 등록 후 트래픽을 정상적으로 수신하려면, 네트워크 인터페이스에 **VIP를 허용된 주소 쌍(Allowed Address Pair)으로 추가**하고, 멤버 서버 내부에서 ARP 커널 파라미터 설정, lo 인터페이스에 VIP 추가, Security Group 규칙 설정이 필요합니다. 자세한 절차는 [로드 밸런서(DSR) 개요](/Network/Load%20Balancer%20(DSR)/ko/overview/)의 멤버 서버 설정 가이드를 참고하시기 바랍니다.
...
로드 밸런서(DSR) 상세 화면의 **상태 확인** 탭에서 현재 설정된 헬스 모니터 정보를 확인하고 변경할 수 있습니다.

<a id='view-dsr-health-monitor'></a>
### 상태 확인 조회
상태 확인 탭에서는 현재 설정된 헬스 모니터의 다음 정보를 확인할 수 있습니다.
...
### 상태 확인 설정 변경
상태 확인 탭에서 **설정 변경** 버튼을 클릭하면 헬스 모니터 설정을 변경할 수 있습니다.
...
> [참고] 헬스 모니터 수 제한<br>
> 하나의 로드 밸런서(DSR)에는 헬스 모니터를 1개만 생성할 수 있습니다.

> [참고] 헬스 모니터 미설정 시 동작<br>
> 헬스 모니터를 설정하지 않은 경우, 멤버의 상태는 자동으로 변경되지 않으며 초기 상태인 `INACTIVE`로 유지됩니다. 트래픽 분산을 위해서는 헬스 모니터 설정을 권장합니다.

> [참고] 헬스 체크 트래픽 허용<br>
> 헬스 체크 요청은 로드 밸런서(DSR)와 동일한 서브넷에 자동 할당된 **헬스 모니터 전용 IP**에서 발송됩니다. 멤버 인스턴스의 Security Group에서 해당 트래픽을 허용해야 헬스 체크가 정상 동작합니다. 자세한 내용은 [로드 밸런서(DSR) 개요](/Network/Load%20Balancer%20(DSR)/ko/overview/)의 Security Group 설정 섹션을 참고하시기 바랍니다.
