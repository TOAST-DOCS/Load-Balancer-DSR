...
> TCP/HTTP 상태 확인는 DSR VIP를 목적지로 요청하므로, 멤버 서버의 lo 인터페이스에 VIP가 설정되어 있지 않으면 해당 패킷이 수신·처리되지 못해 상태 확인가 실패하고 멤버가 `INACTIVE`로 판정됩니다. ICMP 상태 확인는 멤버의 실제 IP로 요청하므로 VIP 설정과 무관하게 연결성만 확인합니다.
...
> - Security Group에서 서비스 포트 및 상태 확인 트래픽 허용
>
> 자세한 절차는 **[로드 밸런서(DSR) 개요](/Network/Load%20Balancer%20(DSR)/ko/overview/)**의 멤버 서버 설정 가이드를 참고하시기 바랍니다.

> [참고] 멤버 초기 상태<br>
> 새로 등록된 멤버의 초기 상태는 `INACTIVE`입니다. 헬스 모니터가 설정된 경우, 상태 확인를 통과하면 자동으로 `ACTIVE` 상태로 전환되어 트래픽을 수신합니다.
...
> [참고] 로드 밸런서(DSR)의 상태는 다음 중 하나로 결정됩니다.<br>
> | 상태 | 의미 |
> |--|--|
> | `ACTIVE` | 정상 동작 중 |
...
> [참고] 상태 확인 트래픽 허용<br>
> 상태 확인 요청은 로드 밸런서(DSR)와 동일한 서브넷에 자동 할당된 **헬스 모니터 전용 IP**에서 발송됩니다. 멤버 인스턴스의 Security Group에서 해당 트래픽을 허용해야 상태 확인가 정상 동작합니다. 자세한 내용은 [로드 밸런서(DSR) 개요](/Network/Load%20Balancer%20(DSR)/ko/overview/)의 Security Group 설정 섹션을 참고하시기 바랍니다.
