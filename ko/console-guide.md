...
!!! tip "알아두기"
    상태 확인 요청은 로드 밸런서(DSR)와 동일한 서브넷에 자동 할당된 상태 확인 전용 IP에서 발송됩니다. 멤버 인스턴스의 Security Groups에서 원격을 해당 서브넷 CIDR로 지정하여 트래픽을 허용해야 상태 확인이 정상 동작합니다. 자세한 내용은 [로드 밸런서(DSR) 개요](/Network/Load%20Balancer(DSR)/ko/overview/)의 Security Groups 설정 섹션을 참고하세요.
